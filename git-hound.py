...
parser.add_argument(
    '--all', default=False,
    type=bool,
    help='Print all URLs, including ones with no pattern match.')
parser.add_argument(
    '--regex', default=False,
    help='Supply a custom regex file')
parser.add_argument(
...
parser.add_argument(
    '--regex-file',
    help='Supply your own regex list (a list of expressions)')
parser.add_argument(
    '--config-file',
...
def search_code(query, sessions, filetype=None):
...
def regex_array(string, array):
  regex = r"("
  for elm in array[:-1]:
    regex += re.compile(elm) + r"|"
  regex += re.compile(array[:-1]) + r")"
  return regex
...
visited = set()
visited_hashes = set()
def print_paths_highlighted(subdomain, paths, sessions, output_file, int_output_file):
  ...
  for path in paths:
    ...
    matches = re.findall(
      r"(sf_username|api_key"
#        + r"|\b4[0-9]{12}(?:[0-9]{3})?\b" # credit card numbers
#        + r"|\b(?!000|666|9\d{2})([0-8]\d{2}|7([0-6]\d))([-]?|\s{1})(?!00)\d\d\2(?!0000)\d{4}\b" # ssn
        + r"|(stage|staging)\." + re.escape(domain) + r"|db_username|db_password"
        + r"|api_key_admin|x\-api\-key|x\-query\-key|insights(\-|\_)key|accounts\/1\/|account\/1\/" # new relic
        + r"|jenkins"
        # + r"|aws_access_key_id"
    ...
    if args.api_keys:
      generic_api_keys = re.findall(r"("
          ...
          # + r"|\b[0-9A-z]{32}\b" # Fastly
          + r"|\b[0-9A-z]{40}\b" # Zendesk
          + r"|\b[0-9a-z]{64}\b" # HelloSign
    ...
    if score > -5:
      ...
      int_output_file.write('https://github.com/' + path + "\n")
      for match in match_set:
        truncated = match[0]
        if len(match[0]) > 35:
          truncated = match[0][:35]
        print('  > ' + truncated)
        int_output_file.write('  > ' + match[0] + "\n")
    else:
...
regex_string = ""
if args.regex_file:
  regex_array = open(args.regex_file).read().split("\n")
  regex_string = regex_array(regex_array)
print(regex_string)
...
output_file = open('git-hound-output.txt', 'w+')
int_output_file = open('git-hound-output-int.txt', 'w+')

for subdomain in subdomains:
  paths = set()
  for file_type in ['SQL', 'Jupyter+Notebook', 'Shell']:
    for path in search_code('"' + subdomain + '"', sessions, filetype=file_type):
      paths.add(path)
  ...
  for filename in files:
    for path in search_code('filename:' + filename + ' "' + subdomain + '"', sessions):
      paths.add(path)
  print_paths_highlighted(subdomain, paths, sessions, output_file, int_output_file)
  time.sleep(5)
  output_file.close()
